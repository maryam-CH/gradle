...
class TaskNameMatchingIntegrationTest extends AbstractIntegrationSpec {
    ...
    def "logs info message for pattern match"() {
        ...
        then:
        outputContains("No exact task with name ‘sanC’ has been found. Checking for abbreviated names.")
        outputContains("Found exactly one task name, that matches the abbreviated name ‘sanC’: 'sanityCheck'.")
        result.assertTaskExecuted(":sanityCheck")
        result.assertTaskNotExecuted(":safetyCheck")
    }
