...
public class ProjectFinderByTaskPath {
    ...
    public ProjectInternal findProject(String projectPath, ProjectInternal startProject) throws ProjectLookupException {
        ...
        if (matchedProject.isPatternMatched()) {
            LOGGER.info("No exact project with name ‘{}’ has been found. Checking for abbreviated names.", originalProjectPath);
            LOGGER.info("Found exactly one project that matches the abbreviated name ‘{}’: '{}'.", originalProjectPath, matchedProject.getProject().getPath());
        } else {
            LOGGER.info("Found exact project with name '{}'", matchedProject.getProject().getPath());
        }
